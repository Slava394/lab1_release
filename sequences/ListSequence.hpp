...
{
protected:
    SharedPtr<LinkedList<T>> dataHolder;
    virtual ListSequence<T> *Clone() = 0;
    virtual ListSequence<T> *Instance() = 0;
public:
    //---Constructors---
    ListSequence() : dataHolder(new LinkedList<T>()) {}

    ListSequence(const T* items, int count) : dataHolder(new LinkedList<T>(items, count)) {}

    ListSequence(const ListSequence<T>& otherListSequence) : dataHolder(new LinkedList<T>(otherListSequence.GetDataHolder())) {}

    explicit ListSequence(const Sequence<T>& otherSequence) : dataHolder(new LinkedList<T>(otherSequence.GetDataHolder())) {}
    ...
    //     result->dataHolder = SharedPtr<LinkedList<T>>(&dataHolder->GetSubList(startIndex, endIndex));
    //     return *result;
    // }
