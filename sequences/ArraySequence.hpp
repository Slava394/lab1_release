...
{
protected:
    SharedPtr<DynamicArray<T>> dataHolder;
    virtual ArraySequence<T> *Clone() = 0;
    virtual ArraySequence<T> *Instance() = 0;
public:
    //---Constructors---
    ArraySequence() : dataHolder(new DynamicArray<T>()) {}

    ArraySequence(const T* items, int count) : dataHolder(new DynamicArray<T>(items, count)) {}

    ArraySequence(const ArraySequence<T>& otherArraySequence) : dataHolder(new DynamicArray<T>(otherArraySequence.GetDataHolder())) {}

    explicit ArraySequence(const Sequence<T>& otherSequence) : dataHolder(new DynamicArray<T>(otherSequence.GetDataHolder())) {}
    ...
    // //---Map-reduce---
    // ArraySequence<T>* Concat(const ArraySequence<T>& otherArraySequence) 
    // {
    ...
    // {
    //     ArraySequence<T>* result = Clone();
    //     result->dataHolder = SharedPtr<DynamicArray<T>>(&dataHolder->GetSubArray(startIndex, endIndex));
    //     return *result;
    // }
